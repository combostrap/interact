...
import htmlToMarkdown from "@/markdown/htmlToMarkdown.tsx";
/**
...
import {getRootResponse, getStaticPaths} from "./handler.tsx";
...
export function parseRenderRequest(request: Request): ContextProps {
    ...
    if (!url.pathname.endsWith(URL_RSC_POSTFIX)) {
        return {
            meta: {
                isRscRequest: false,
                isRscActionRequest: isAction,
                isMarkdownRequest: isMarkdownRequest
            },
            request,
            url,
            response: {}
...
export default async function handler(request: Request): Promise<Response> {
    ...
    if (contextProps.meta.isRscActionRequest) {
     ...
     * that asks for a Rsc format (ie by adding the {@link URL_RSC_POSTFIX} in the URL)
