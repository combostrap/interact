...
async function renderStatic(config: ResolvedConfig) {
     ...
     */
    let rscIndexFilePath = path.join(rscEnv.build.outDir, 'index.js')
    if (!fs.existsSync(rscIndexFilePath)) {
        // in the generated bundle, the file extension is hardcoded in js
        console.log(`Your package.json should have "type": "module"`);
    }
    let rscModuleHref = pathToFileURL(rscIndexFilePath).href;
    console.log(`Note: Trying to import the Rsc module bundle: ${rscModuleHref}`);
    ...
    const entryRscModule: typeof import('@/rsc/server/entry.rsc.tsx') = await Promise.race([
        // bust the cache on the URL
        importModule(/* @vite-ignore */ rscModuleHref + '?t=' + Date.now()),
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('import() hung — never settled, can be caused by a deadlock between 2 different import')), 5000)
        )
    ])

    console.log(`Note: Rsc module bundle imported successfully`);
