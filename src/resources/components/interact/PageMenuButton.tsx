...
export const PageButtonContext = ({query: localQuery, children}: OpenContextProps) => {
    ...
    // Set local Query if it changes
    useEffect(() => {
        setQuery(localQuery);
    }, [localQuery]);

    // Set default query if there is no value
    useEffect(() => {
        if (currentHref == null) {
            return;
        }
        if (localQuery != null && localQuery != "") {
            return;
        }
...
export const OpenAsPdfAnchor = ({children, href, hideSvg = false, ...props}: OpenUiInExternalAnchor & {
    ...
    // _blank should open in a new tab, but it's not guaranteed
    return (
        <a
            href={href}
            rel="noopener"
