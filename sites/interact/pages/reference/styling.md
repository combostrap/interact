...
[Layouts](layout.md) adds a `prose` class when the [prose frontmatter meta](frontmatter.md#list-of-metadata) is `true`.

This meta is automatic set to `true` for [markdown](markdown.md) and [CMS](cms.md) content.

You can enable/disable it by setting the `prose` meta to a value.

You can then use it for styling:

* in your [Global CSS File](#global-css-file)
* or with [tailwindcss-typography class](https://github.com/tailwindlabs/tailwindcss-typography)
